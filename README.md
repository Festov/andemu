...
| BIOS / ускорение | VT-x / AMD-V | **AEHD** (драйвер эмулятора) или WHPX, если IT его даёт |
| Java | не нужна заранее | при первом запуске сама скачается JDK 17 (Temurin) в `runtime\jdk` |

**Важно:** «Виртуализация: Включено» в Диспетчере задач — только VT-x. Компонента «Платформа гипервизора Windows» на корпоративных ПК часто **нет** — тогда ставьте **AEHD**:

1. Сделайте setup (скачается SDK).
2. Запустите **`Install-AEHD.bat`** от имени администратора (UAC).
3. Если AEHD ругается на Hyper-V — нужен IT (дать WHPX **или** отключить Hyper-V / Memory Integrity для AEHD).

Проверка: `runtime\android-sdk\emulator\emulator.exe -accel-check`
