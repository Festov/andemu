...
| BIOS | VT-x / AMD-V включены | + **Платформа гипервизора Windows (WHPX)** или драйвер AEHD |
| Java | не нужна заранее | при первом запуске сама скачается JDK 17 (Temurin) в `runtime\jdk`, если на ПК нет JDK 17+ |

**Важно:** «Виртуализация: Включено» в Диспетчере задач — это только VT-x в процессоре. Для эмулятора Android дополнительно нужен **WHPX** или **Android Emulator Hypervisor Driver (AEHD)**. Setup/start попробуют поставить их сами (нужны права администратора / UAC); после WHPX иногда требуется перезагрузка.
